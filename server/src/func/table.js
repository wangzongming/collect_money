...
const table = {
  async add({
    ...
    try {
      ...
      if (otherParams.includes("create_user")) {
        params.create_user = ctx.state.username;
      }
      //增加创建者id
      if (otherParams.includes("create_user_id")) {
        params.create_user_id = ctx.state.uid;
      }
      //增加创建时间
  ...
  async update({
    ...
    try {
      const params = await think.newParams(ctx, rules);
      if (!think.isObject(params)) {
        ctx.body = {
